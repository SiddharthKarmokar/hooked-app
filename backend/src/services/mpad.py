...
def compute_reranked_score(hook, base_score, max_views=1):
    ...
    recency_score = math.exp(-days_old / 7)  # tau = 7 days
    ...
    return total_score#higher the better
...
async def get_candidate_hooks(user_interest_vector, N=100, lambda_param=0.7):
    ...
    top_k = scored_hooks[:K]  # Over-generate to give room for diversification

    # Perception-aware diversification via MMR
...
async def generate_mpad_feed(user_id, N=N_VALUE):
    ...
    diversified = await get_candidate_hooks(user_interest_vector=interest_vector, N=N, lambda_param=MMR_LAMBDA)
